...
def step_expense_is_shown(context, amount, description, date):
    ...
    # Re-query rows AFTER the table has rendered
    ...
    for row in rows:
        ...
        # Safety check (avoid header rows or malformed rows)
        if len(cells) < 3:
            continue

        row_date = cells[0].text.strip()
        row_amount = cells[1].text.strip()
        row_description = cells[2].text.strip()

        if (
          row_date == date
          and expected_amount in row_amount
          and row_description == description
        ):
            found = True
            break

    print("ROW DEBUG:", [cell.text for cell in cells])
